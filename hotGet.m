function  [userVoteMat,user_id,item_id] = hotGet( )%numUsersLimit )
...
%     fileReadMat_mat = fileReadMat_mat_temp(ix,:);
    ...
    for row_idx = 1:size(fileReadMat_mat,1)
        if fileReadMat_mat(row_idx,1) ~= user_id_temp || fileReadMat_mat(row_idx,2) ~= brand_id_temp
            ...
            if transformVote(fileReadMat_mat(row_idx,3),fileReadMat_mat_date(row_idx)) > score_temp
                score_temp = transformVote(fileReadMat_mat(row_idx,3),fileReadMat_mat_date(row_idx)) + score_temp;%*0.6;
            else
...
ind1=find(sum(userVoteBinary)<3);
item_id = [1:size(userVoteBinary,2)];
item_id(ind1) = [];
userVoteMat(:,ind1)=[];

userVoteBinary=spones(userVoteMat);
ind2=find(sum(userVoteBinary,2)<3);
